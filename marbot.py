...
async def logUserBot():
    load_dotenv()
    api_id = int(25723244)
    api_hash = "822ca6a8862789fc7d2597f0c5b889f6"
    phone_number = "51949910622"
    ...
    await client.send_message("@spmMarBot", f'<b>Bot encendido</b>', parse_mode="HTML")
    spammer_group = int("-4144262404")
    spammer_group2 = int("-4117262025")
    ...
    while True:
        ...
        try:
            await client.send_message("@spmMarBot", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>",parse_mode="HTML")
        except:
            pass
                    ...
                    if i['group_name'] in ["🇵🇪🧑🏼‍💻𝙋𝙀𝙍𝙐 𝘼𝙔𝙐𝘿𝘼🧑🏼‍💻🇵🇪","BINS PERU🇵🇪","COMPRA Y VENTA🇵🇪PERU","STREAMING LATINO GO","🇵🇪🧑🏼‍💻𝘽𝙐𝙎𝙌𝙐𝙀𝘿𝘼𝙎 𝙋𝙀𝙍𝙐🧑🏼‍💻🇵🇪"]:
                        ...
                        for message_spam in special_messages_list:
                            ...
                            try:
                                ...
                                await client.send_message("@spmMarBot", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}',parse_mode="HTML")
                                resultado = False
                            if resultado:
                                await client.send_message("@spmMarBot", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>',parse_mode="HTML")  
                        ...
                        for message_spam in messages_list:
                            ...
                            if j==4: break
            await client.send_message("@spmMarBot", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
